...
import { checkDomainAssociationsModify } from '@/supabase/org_details';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (user) {
      const { domain_name, org_id } = await req.json();

      const result = await checkDomainAssociationsModify(domain_name, org_id);
